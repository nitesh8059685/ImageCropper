...
public struct Quadrilateral: Transformable {
    ...
    /// A point that specifies the midpoint of the top edge of the quadrilateral.
    public var midTop: CGPoint {
        return CGPoint(x: (topLeft.x + topRight.x) / 2, y: (topLeft.y + topRight.y) / 2)
    }

    /// A point that specifies the midpoint of the right edge of the quadrilateral.
    public var midRight: CGPoint {
        return CGPoint(x: (topRight.x + bottomRight.x) / 2, y: (topRight.y + bottomRight.y) / 2)
    }

    /// A point that specifies the midpoint of the bottom edge of the quadrilateral.
    public var midBottom: CGPoint {
        return CGPoint(x: (bottomRight.x + bottomLeft.x) / 2, y: (bottomRight.y + bottomLeft.y) / 2)
    }

    /// A point that specifies the midpoint of the left edge of the quadrilateral.
    public var midLeft: CGPoint {
        return CGPoint(x: (bottomLeft.x + topLeft.x) / 2, y: (bottomLeft.y + topLeft.y) / 2)
    }

    public var description: String {
        return "topLeft: \(topLeft), topRight: \(topRight), bottomRight: \(bottomRight), bottomLeft: \(bottomLeft), midTop: \(midTop), midRight: \(midRight), midBottom: \(midBottom), midLeft: \(midLeft)"
    }
    ...
    var top: Double {
        return Double(topLeft.distanceTo(point: topRight))
    }

    /// The bottom edge of the quadrilateral
    var bottom: Double {
        return Double(bottomLeft.distanceTo(point: bottomRight))
    }

    /// The left edge of the quadrilateral
    var left: Double {
        return Double(topLeft.distanceTo(point: bottomLeft))
    }

    /// The right edge of the quadrilateral
    var right: Double {
        return Double(topRight.distanceTo(point: bottomRight))
    }
    ...
    init(topLeft: CGPoint, topRight: CGPoint, bottomRight: CGPoint, bottomLeft: CGPoint) {
        self.topLeft = CGPoint(x: topLeft.x, y: topLeft.y )
        self.topRight = CGPoint(x: topRight.x, y: topRight.y )
        self.bottomRight = CGPoint(x: bottomRight.x, y: bottomRight.y )
        self.bottomLeft = CGPoint(x: bottomLeft.x, y: bottomLeft.y )
    }
